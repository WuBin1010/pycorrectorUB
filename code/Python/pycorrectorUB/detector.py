...
class Detector(object):
    ...
    def __init__(self, language_model_path=config.language_model_path,
        ...
        self.stopwords = None
        self.tokenizer = None
        self.segment_word_confusion = corrector.segment_word_confusion  # 判断custom_confusion白名单词典时，是否先分词。
    ...
    def _get_maybe_error_index_by_stddev(scores, n=2):
        """
        取疑似错字的位置，通过平均值上下n倍标准差之间属于正常点，否则就算异常点，候选错字
    ...
    def detect_short(self, sentence, start_idx=0):
        ...
        # 初始化
        self.check_detector_initialized()
        # 自定义混淆集加入疑似错误词典, 首先使用混淆集数据，即白名单数据。  (删除：沈思 沉思)
        word_list = jieba.lcut(sentence)  # 使用结巴分词,实测可以把大部分confuse配置的词分出来，效果较好。
        logger.debug("~~~ custom_confusion. word_list:" + str(word_list))
        for confuse in self.custom_confusion:
            # 先分词，判断白名单词是否在分词后的word_list中。 update by WuBin
            if self.segment_word_confusion is False or (self.segment_word_confusion is True and confuse in word_list):
                idx = sentence.find(confuse)  # 句子中包含白名单的字就纠错？不合理，应该先分词。不然会误分词。但是可能分词不正确，无法匹配白名单。
                if idx > -1:
                    maybe_err = [confuse, idx + start_idx, idx + len(confuse) + start_idx, ErrorType.confusion]
                    self._add_maybe_error_item(maybe_err, maybe_errors)
                    logger.debug("~~~ custom_confusion. sentence:" + sentence)
                    logger.debug("~~~ custom_confusion. maybe_err:" + str(maybe_err))
                    logger.debug("~~~ custom_confusion. maybe_errors:" + str(maybe_errors))

        if self.is_word_error_detect:
            logger.debug("~~~ is_word_error_detect. sentence:" + sentence)
            # 切词
            tokens = self.tokenizer.tokenize(sentence)
            logger.debug("~~~ is_word_error_detect. tokens:" + str(tokens))
            # 未登录词加入疑似错误词典
            for token, begin_idx, end_idx in tokens:
                # pass filter word
                if self.is_filter_token(token):  # 按规则过滤疑似错误。
                    continue
                # pass in dict
                if token in self.word_freq:  # 按词典过滤疑似错误，在data/custom_word_freq.txt 中定义
                    continue
                maybe_err = [token, begin_idx + start_idx, end_idx + start_idx, ErrorType.word]
                logger.debug("~~~ is_word_error_detect. maybe_err:" + str(maybe_err))  # 未登录词都是候选集
                self._add_maybe_error_item(maybe_err, maybe_errors)
                logger.debug("~~~ is_word_error_detect. maybe_errors:" + str(maybe_errors))

        if self.is_char_error_detect:
            logger.debug("~~~ is_char_error_detect, use ngram language model. sentence:" + sentence)
            # 语言模型检测疑似错误字
            try:
                ngram_avg_scores = []
                for n in [2, 3]:
                    ...
                    for i in range(len(sentence) - n + 1):
                        word = sentence[i:i + n]
                        score = self.ngram_score(list(word))  # 按字符取2阶，3阶Ngram的值。
                        logger.debug("~~~ is_char_error_detect. word:" + str(word) + ", score:" + str(score))
                        scores.append(score)
                    if not scores:
                        continue
                    # 移动窗口补全得分
                    ...
                    logger.debug("~~~ is_char_error_detect. ngram_avg_scores:" + str(ngram_avg_scores))

                if ngram_avg_scores:
                    # 取拼接后的n-gram平均得分， 把2阶的ngram值于3阶的值求平均
                    sent_scores = list(np.average(np.array(ngram_avg_scores), axis=0))
                    logger.debug("~~~ is_char_error_detect. sent_scores:" + str(sent_scores))
                    # 取疑似错字信息
                    for i in self._get_maybe_error_index(sent_scores):
                        ...
                        maybe_err = [token, i + start_idx, i + start_idx + 1,
                                     ErrorType.char]
                        logger.debug("~~~ is_char_error_detect. maybe_err:" + str(maybe_err))
                        self._add_maybe_error_item(maybe_err, maybe_errors)
                        logger.debug("~~~ is_char_error_detect. maybe_errors:" + str(maybe_errors))
